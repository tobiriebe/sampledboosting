...
if (length(commandArgs(trailingOnly = TRUE))>0) {
  ...
  dataFile <- args[6]                 #datafile to load
  localRun <- FALSE
} else {
  ###### local testing
...
# simulations is already called 'simulations' in the datafile
...
outLabel <- paste(dataFile,"_OUT_", nOuterFolds, "_", redSteps, "_", sampleRatio, "_", fixedMstop, "_", fixedNu, ".rda", sep = "")
#save all variables from input file, parameters and output
save(Y, Ynoise, yBin, yBinNoise, originalX, originalXnoise, coeffs, predictors, kappa, samples, simulations, 
     nOuterFolds, redSteps, sampleRatio, fixedMstop, fixedNu,
     offsetFinal, predModelList, offsetFinalClass, predModelListClass, predictionVector, predictionVectorClass,
     offsetFinalNoise, predModelListNoise, predictionVectorNoise, offsetFinalClassNoise, predModelListClassNoise, predictionVectorClassNoise,
     file = outLabel)

# ############## DEBUG PLOTS ##################
# par(mfrow=c(4,2))
# plot(predictionVector[1,1,] ~ Y[,1], main = paste("cor=", cor(predictionVector[1,1,], Y[,1])))
# plot(predictionVector[1,1,] ~ Ynoise[,1], main = paste("cor=", cor(predictionVector[1,1,], Ynoise[,1])))
# plot(predictionVector[1,100,] ~ Y[,1], main = paste("cor=", cor(predictionVector[1,100,], Y[,1])))
# plot(predictionVector[1,100,] ~ Ynoise[,1], main = paste("cor=", cor(predictionVector[1,100,], Ynoise[,1])))
# plot(predictionVectorNoise[1,1,] ~ Y[,1], main = paste("cor=", cor(predictionVectorNoise[1,1,], Y[,1])))
# plot(predictionVectorNoise[1,1,] ~ Ynoise[,1], main = paste("cor=", cor(predictionVectorNoise[1,1,], Ynoise[,1])))
# plot(predictionVectorNoise[1,100,] ~ Y[,1], main = paste("cor=", cor(predictionVectorNoise[1,100,], Y[,1])))
# plot(predictionVectorNoise[1,100,] ~ Ynoise[,1], main = paste("cor=", cor(predictionVectorNoise[1,100,], Ynoise[,1])))
# par(mfrow=c(1,1))

#### evaluation scores  ########### THIS SHOULD LATER BE MOVED TO THE EVALUATION SCRIPT/FUNCTION ##################
#### evaluation scores  ########### THIS SHOULD LATER BE MOVED TO THE EVALUATION SCRIPT/FUNCTION ##################
#regression: sum of absolute errors
# sumAbsErrors <- rep(0, redSteps)
# sumAbsErrorsNoiseX <- rep(0, redSteps)
# sumAbsErrorsNoiseY <- rep(0, redSteps)
# sumAbsErrorsNoiseXNoiseY <- rep(0, redSteps)
# #################################################################################
# for (simulation in 1:simulations) {
# 
#   residuals <- colSums(abs(t(predictionVector[simulation,,])-Y[,simulation]))/simulations
#   sumAbsErrors <- sumAbsErrors + residuals
# 
#   residuals <- colSums(abs(t(predictionVectorNoise[simulation,,])-Y[,simulation]))/simulations
#   sumAbsErrorsNoiseX <- sumAbsErrorsNoiseX + residuals
# 
#   residuals <- colSums(abs(t(predictionVector[simulation,,])-Ynoise[,simulation]))/simulations
#   sumAbsErrorsNoiseY <- sumAbsErrorsNoiseY + residuals
# 
#   residuals <- colSums(abs(t(predictionVectorNoise[simulation,,])-Ynoise[,simulation]))/simulations
#   sumAbsErrorsNoiseXNoiseY <- sumAbsErrorsNoiseXNoiseY + residuals
# }
# par(mfrow=c(2,2))
# plot(sumAbsErrors)
# plot(sumAbsErrorsNoiseX)
# plot(sumAbsErrorsNoiseY)
# plot(sumAbsErrorsNoiseXNoiseY)
# par(mfrow=c(1,1))
###################################################################################################################
